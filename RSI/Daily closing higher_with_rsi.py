                ...
                if "Symbol" not in self.line:
                    ...
                    with open(path_to_historical_data + self.line[self.column_no_of_ticker] + ".csv", 'r') as g:
                        ...
                        if (prev_day_date == date):
                            ...
                            self.logger.critical("Moving to the next stock for processing")
                            continue
                        ...
                        while index < len(self.rowslist):
                            ...
                            if bought == 0 and do_not_buy == 0:
                                ...
                                # previous_rsi=float(rsi[index-2])
                                # current_rsi=float(rsi[index-1])
                                # if current_rsi>30 and previous_rsi <30 :
                                # if current_rsi < 30:
                                if True:
                                    self.logger.info("Index before checking for the candles: " + str(index))
                                    current_is_green= True if ha_close[index-1]>ha_open[index-1] else False
                                    previous_is_green= True if ha_close[index-2]>ha_open[index-2] else False
