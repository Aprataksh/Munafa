...
class OHL():
    ...
    def check_price_volatility(self, index_open, index_close):
        i = index_open + 1
        while i <= index_close:
            vol_high = (float(self.rowslist[i][2]) - float(self.rowslist[index_open][2])) / float(
                self.rowslist[index_open][2])
            vol_low = (float(self.rowslist[i][3]) - float(self.rowslist[index_open][3])) / float(
                self.rowslist[index_open][3])
            if abs(vol_high) > self.max_price_volatility or abs(vol_low) > self.max_price_volatility:
                print("Stock " + self.line[0] + " rejected on date " + str(self.rowslist[index_open])[
                                                                       2:18] + " due to price volatility\n")
                print("Volatility high" + str(vol_high) + " volatility low " + str(vol_low)
                + "High: " + self.rowslist[i][2] + " Low: " + self.rowslist[i][3] + " Open candle high: "
                + self.rowslist[index_open][2] + " Open candle low : " + self.rowslist[index_open][3] + "\n")
    ...
    def check_volume_volatility(self, index_open, index_close):
        ...
        while i <= index_close:
            if float(self.rowslist[i][5]) >= self.max_volume_volatility * float(self.rowslist[i - 1][5]):
                print("Stock " + self.line[0] + " rejected on date " + str(self.rowslist[i])[
                                                                       2:18] + " due to volume volatility\n")
                print("Max volume volatility" + str(self.max_volume_volatility) + "current candle volume: "
                      + self.rowslist[i][5] + " previous candle volume: " + self.rowslist[i-1][5]  + "\n")
    ...
    def OHL(self):
        ...
        path_to_stock_master_list = "C:/Users/Rohit/Python_source_code/output/OHL/version2/daily_closing_higher_output.csv"
        ...
        path_to_output_directory = "C:/Users/Rohit/Python_source_code/output/OHL/version2/"
                        ...
                        with open(path_to_historical_data + self.line[self.column_no_of_ticker] + ".csv", 'r') as g:
                            ...
                            while index < len(self.rowslist):
                                ...
                                if date + " 11:" in row[0] and c == 0:
                                    print("No data for stock " + self.line[self.column_no_of_ticker] + " on date "
                                          + str(row)[2:12] + "\n")
                                    c = 2
                                if c == 1:
                                    ...
                                    if 0 <= ((close_price - open_price) / open_price) <= self.max_deviation_from_open:
                                        ...
                                        if f == 0:
                                            ...
                                            print("Stock price of " + self.line[self.column_no_of_ticker] +
                                                  " fell or increased too much on date " + str(row)[2:12] + "\n")

                                            print("Close Price " + str(close_price) + "Open price" + str(open_price) +
                                                  " maximum deviation from open " + str(self.max_deviation_from_open)
                                                  + "\n")
                            ...
                            print("Total buy = " + str(self.total_cost) + " total sell = " + str(self.total_sell) +
                                  " and total profit = " + str(self.total_sell - self.total_cost) + " for stock " + self.line[2] + "\n")
                            ...
                            with open(path_to_output_directory + self.line[self.column_no_of_ticker] + ".csv",
                                      'w', newline="") as f:
                                writer = csv.writer(f)
                                for row in rows:
                                    writer.writerow(row)
        ...
        print("Overall buys = " + str(self.c_overall_buy_trans) + " Overall wins = " + str(self.overall_c_wins) +
              " Overall sell EOD = " + str(self.overall_c_sellEod) + " Overall stoploss = " + str(self.overall_c_stoploss))
def main():
    ...
    stop_loss = -0.01
    obj = OHL(max_price_volatility, max_deviation_from_open, max_volume_volatility, max_capital_for_single_buy,
