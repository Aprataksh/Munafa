...
def get_daily_closing_low(no_of_days, output_folder):
    ...
    output_filename = "scanner_daily_closing_lower_output.csv"
    log_filename = "strategy_log.log"
        ...
        for i in range(1,len(date_day)-no_of_days+1):
            ...
            for j in range(0,no_of_days-1):
                ...
                if float(close_day[current_day]) < float(open_day[current_day])\
                    ...
                    day_change = float(open_day[previous_day])-float(close_day[previous_day])
                    if (float(open_day[current_day]) - float(close_day[previous_day])) < .98 * day_change\
                            and float(high_day[current_day]) < float(open_day[previous_day]) :
...
def main():
    ndays = 2
    output_folder = "Check/"
    get_daily_closing_low(ndays, output_folder)
