...
class Buy_sell():
    ...
    def buy_stocks(self, row):
        ...
        checkdate = row[self.datetime_column][2:11] + purchase_cutoff_time[:4] + ":00"
        ...
        curr_date = datetime.strptime(row[self.datetime_column][2:18], "%y-%m-%d %H:%M:%S")
        if curr_date < checkdate:
            ...
            if bought != 0:
                ...
                self.logger.info("Stock " + self.line[self.column_no_of_ticker] + " bought " + str(bought) + " shares at " + str(purchase_cost) +
            	    " price at date " + str(row)[2:12] + " at time " + str(row)[12:18] + "\n")
                ...
                # maximum capital
                self.logger.error("Could not buy stock " + self.line[0] + " on date " + str(row)[2:12] +
            	    " due to insufficient daily stock fund\n")
            return bought
        else:
            self.logger.error(
                "Could not buy stock " + self.line[0] + " on date " + str(row)[2:12] +
                " due to cut-off time for purchase\n")
            return 0

    def sell_stock_due_to_price_check(self, bought, purchase_price, purchase_date, row):
        ...
        if per >= self.target_price:
            ...
            self.logger.info("Stock " + self.line[self.column_no_of_ticker] + " sold " + str(bought) + " shares at " + str(current_close_price) 
            	+ " price at date " + str(row)[2:12] + " at time " + str(row)[13:18] + "\n")
    ...
    def initial_target_met(self, purchase_price, row):
        current_close_price = float(row[self.close_column])
        per = self.position_flag * ((float(row[self.close_column]) - purchase_price) / purchase_price)
    ...
    def trailing_stoploss(self, initial_stoploss, purchase_price, row):
        per = self.position_flag * ((float(row[self.close_column]) - purchase_price) / purchase_price)
        if (per - self.trailing_sl_diff) > self.stop_loss:
            self.stop_loss = (per - self.trailing_sl_diff)

    def sell_stock_due_to_stop_loss(self, bought, purchase_price, purchase_date, row):
        current_close_price = float(row[self.close_column])
        per = self.position_flag * ((current_close_price - purchase_price) / purchase_price)
        if per <= self.stop_loss:
            ...
            self.logger.info("Stop Loss Sale: Stock " + self.line[self.column_no_of_ticker] + " sold " + str(bought) + " shares at " 
            	+ str(current_close_price) + " price at date " + str(row)[2:12] + " at time " + str(row)[13:18] + "\n")
    ...
    def sell_stock_at_end_of_day(self, bought, purchase_price, purchase_date,  row):
        current_close_price = float(row[self.close_column])
        sell_amount = bought * current_close_price
        self.total_sold += bought
        self.total_sell = self.total_sell + sell_amount
